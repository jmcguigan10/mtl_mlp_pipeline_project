...
from .lebedev17_fallback import PTS_X, PTS_Y, PTS_Z, WEIGHTS

try:
    from scipy.integrate import lebedev_rule as scipy_lebedev_rule
except Exception:
    scipy_lebedev_rule = None


def _build_lebedev_rule_017(dtype: torch.dtype = torch.float32):
    if scipy_lebedev_rule is not None:
        # SciPy returns Cartesian points with dOmega weights (sum = 4*pi).
        pts, weights = scipy_lebedev_rule(17)  # pts shape: [3, N], weights shape: [N]
        pts_t = torch.as_tensor(pts, dtype=dtype)
        w = torch.as_tensor(weights, dtype=dtype)
    else:
        # Python 3.9 + SciPy<=1.13 do not expose scipy.integrate.lebedev_rule.
        # Use a static order-17 table generated from SciPy to preserve behavior.
        pts_t = torch.stack(
            [
                torch.tensor(PTS_X, dtype=dtype),
                torch.tensor(PTS_Y, dtype=dtype),
                torch.tensor(PTS_Z, dtype=dtype),
            ],
            dim=0,
        )
        w = torch.tensor(WEIGHTS, dtype=dtype)
    dir_x = pts_t[0].contiguous()
    dir_y = pts_t[1].contiguous()
    dir_z = pts_t[2].contiguous()
    return dir_x, dir_y, dir_z, w


class Box3DHeuristic(nn.Module):
    '''
    Implements a Box3D-like mixing baseline with Lebedev quadrature.
    ...
    def __init__(self, NF: int, resol_theta: int = 21, resol_phi: int = 41):
        ...
        # Legacy (theta, phi) args are kept for call-site compatibility.
        # Quadrature is now fixed to Lebedev order 17 (110 points).
        vx, vy, vz, w = _build_lebedev_rule_017(dtype=torch.float32)
    ...
    def _extract_species_moments(self, F4: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, F4_in: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
